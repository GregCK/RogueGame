...
onready var wallsNode = $YSort/WallsNode
var player
...
class RoomPlaceholder:
	...
	func _init(xIndex, yIndex, room_type):
		x = xIndex
		y = yIndex
		roomType = room_type
...
#		check to see if the random direction has a potencial room
		if floorGrid[potencialRoomXCoor][potencialroomYCoor] == null:
			...
			if i == amountOfRooms - 1:
				newPlaceHolder = RoomPlaceholder.new(potencialRoomXCoor, potencialroomYCoor, END)
			else:
				newPlaceHolder = RoomPlaceholder.new(potencialRoomXCoor, potencialroomYCoor, REGULAR)
...
#	room.init(isInitialRoom, newRoom.x, newRoom.y, hasDoors)
	room.init(newRoom.roomType, newRoom.x, newRoom.y, hasDoors)
	wallsNode.add_child(room)
...
func add_initial_placeholder_room(x,y):
	var newPlaceHolder = RoomPlaceholder.new(x,y, INITIAL)
